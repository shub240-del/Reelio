...
describe("schema validation", () => {
  it("accepts a well formed removeRanges op", () => {
    expect(editOpSchema.safeParse({ type: "removeRanges", ranges: [{ start: 0, end: 1 }] }).success).toBe(true);
  });

  it("rejects an unknown op type", () => {
    expect(editOpSchema.safeParse({ type: "explode", ranges: [] }).success).toBe(false);
  });

  it("rejects negative times", () => {
    expect(editOpSchema.safeParse({ type: "removeRanges", ranges: [{ start: -1, end: 1 }] }).success).toBe(false);
  });

  it("rejects an empty range list", () => {
    expect(editOpSchema.safeParse({ type: "removeRanges", ranges: [] }).success).toBe(false);
  });
  ...
  it("rejects a plan with no summary", () => {
    expect(editPlanSchema.safeParse({ summary: "", operations: [] }).success).toBe(false);
  });

  it("caps caption text length", () => {
    const long = "x".repeat(501);
    expect(
      editOpSchema.safeParse({ type: "addCaptions", cues: [{ text: long, startTime: 0, endTime: 1 }] }).success,
    ).toBe(false);
  });
});
...
describe("applyEditOps", () => {
  it("executes removeRanges through the engine", () => {
    const res = applyEditOps(base(), ASSETS, [{ type: "removeRanges", ranges: [{ start: 2, end: 4 }] }]);
    expect(timelineDuration(res.clips)).toBe(4);
    expect(res.applied).toHaveLength(1);
  });

  it("executes keepRanges as the inverse of removeRanges", () => {
    const res = applyEditOps(base(), ASSETS, [{ type: "keepRanges", ranges: [{ start: 0, end: 2 }] }]);
  ...
  it("skips an op that references a missing clip instead of throwing", () => {
    const res = applyEditOps(base(), ASSETS, [{ type: "splitClip", clipId: 999, atTime: 1 }]);
  ...
  it("records a split point outside the clip as skipped", () => {
    const res = applyEditOps(base(), ASSETS, [{ type: "splitClip", clipId: 1, atTime: 9 }]);
  ...
  it("routes captions to side effects without touching the timeline", () => {
    const res = applyEditOps(base(), ASSETS, [
      { type: "addCaptions", cues: [{ text: "hello", startTime: 0, endTime: 1 }], replaceExisting: true },
    ]);
  ...
  it("routes markers to side effects", () => {
    const res = applyEditOps(base(), ASSETS, [
      { type: "addMarkers", markers: [{ time: 2, label: "scene", color: "#fff" }], replaceExisting: false },
    ]);
  ...
  it("routes audio filters to side effects", () => {
    const res = applyEditOps(base(), ASSETS, [
      { type: "setAudioFilter", highPassHz: 80, noiseGateDb: -40, gainDb: 0, normalize: true },
    ]);
  ...
  it("leaves the timeline valid after a hostile plan", () => {
    ...
    for (const c of res.clips) {
      ...
      expect(c.sourceStart + c.duration).toBeLessThanOrEqual(ASSETS[1].duration + 1e-6);
  ...
  it("does not mutate the input timeline", () => {
    ...
    applyEditOps(input, ASSETS, [{ type: "removeRanges", ranges: [{ start: 0, end: 2 }] }]);
